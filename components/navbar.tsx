      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 md:px-8 py-4">
          <Link href="#hero" aria-label="Queens Cosmetics inicio">
            <QueensLogo variant="default" size={80} />
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {NAV_LINKS.map((l) => (
              <a
                key={l.href}
                href={l.href}
                className="text-sm font-semibold transition-all relative group hover:[text-shadow:0_0_10px_rgba(212,175,55,0.8),0_0_20px_rgba(212,175,55,0.4)]"
                style={{ color: "#D4AF37" }}
              >
                {l.label}
